...
class DealAccountService
{
    ...
    public function createDealAndAccount(array $validated): JsonResponse
    {
        ...
        if ($existingAccount) {
            if ($this->isAccountDataMismatched($existingAccount, $validated)) {
                return response()->json([
                    ...
                    'errors' => [
                        'account_name' => [
                            __('An account with that name already exists'),
                        ],
                    ],
                ], 400);
            }
    ...
    private function createDeal(string $accessToken, array $validated, string $accountId): JsonResponse
    {
